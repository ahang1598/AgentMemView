...
- [2026-08-12] 代理配置页 + claude-code 合并语义（feat/proxy-settings-page）DONE | lint/typecheck/test/build 全绿 | 用户反馈二问题：(1) claude-code 适配器改为合并语义：只覆写管理的键（BASE_URL/AUTH_TOKEN/三模型/窗口/超时/非必要流量），其余 env 与 hooks 等原样保留；新增 claudeEnv 选项（onboard/apply 同步支持），TRAFFIC 写数字 1；幂等判断升级为全部管理键匹配；新增 2 个合并回归测试（无关键保留+二次更新不破坏）；(2) 代理/接入配置独立页面 /settings/proxy：上游地址表单+生成启动命令、agent 接入表单（含密钥/模型/窗口/超时/禁用非必要流量，留空不改动）、接入/还原；设置页瘦身为入口链接；侧边栏新增导航；api.applyOnboard 扩展 claudeEnv；新增 2 个 UI 测试；实测：apply 端点幂等返 already configured（8 键全匹配）、settings.json 键集与类型正确（TRAFFIC=Int32）、claude -p 经代理真实回复 SETTINGS-OK。
- [2026-08-12] T3 精炼重试成功（fix/refine-flash-retry）DONE | 用户指示换 glm-4.7/5.2 重试；实测智谱账户付费模型（4.5/4.6/4.7/5.2）chat/completions 全部 429，免费 glm-4-flash 200 → 能力配置切 glm-4-flash；首轮仍无事实 → 直测发现 flash 输出被 ```json fence 包裹致 JSON.parse 失败静默降级 → 新增 parseLlmJson（提取首个 JSON 对象）+ 回归测试；重跑真实 claude -p → 任务队列消化无新 DLQ，`GET /memories` 真实产出 L1 事实（pnpm workspaces / Friday 部署，LLM 抽取）；T1-T7 全部通过，全链路真实闭环；e2e 文档同步更新。
- [2026-08-12] 真实 E2E 验证与缺陷修复（fix/e2e-real-claude-proxy）DONE | lint/typecheck/test/build 全绿 | 真实调用 Claude Code 2.1.226 经代理打智谱网关的全链路验证（docs/e2e-manual-verification.md 含用例清单 T1-T7 与实测证据）；实测发现并修复 7 个真实缺陷：(1) Expect/TE 头被 undici 拒绝 → 转发剥离；(2) gzip 双重解压 → 响应剥离 content-encoding/length；(3) 首启无默认空间致代理静默降级 → bootstrapDefaultTenants 幂等引导；(4) L1 精炼从未接线（无触发方+worker 未装配）→ L0 写回去重触发 + startHttpServer 装配 worker（策略热配置，429/5xx 退避重试）；(5) mem: 指令在真实 Agent 下失效（tool_result 追加为更靠后 user 消息）→ 扫描全部 user 消息；(6) 502 无诊断 → 透传 cause；(7) port 0 端口回报。新增回归：bootstrap/refine-wiring/forwardHeaders + llm 重试策略更新；实测结果：T1/T2/T4/T5/T6/T7 通过，T3 管线接通但智谱 chat/completions 配额 429（直连同样 429，非代码问题，启发式全链路已由回归证明）。
- [2026-08-12] 统一配置页（fix/unified-external-services-settings）DONE | lint/typecheck/test/build 全绿 | 用户需求：AgentMemView 自身调用的外部 API（地址/key）要在设置页展现并可手动配置，不割裂；实现：(1) CapabilityCard 增强：表单回填已有配置、apiKey 掌码+显示开关、字段中文标签、清空并停用（存 null 热停用）、状态中文徽章；(2) 设置页新增「外部服务（AgentMemView 自身调用的 API）」区块：六能力列表+配置表单，保存走 PUT /config capability.<key>（与能力中心同契约，热生效），并文案澄清与代理上游的区别；(3) 能力中心页同步新签名并标注两处等价；新增 3 个 UI 测试（回填/保存契约/清空停用）；浏览器实测：配置 LLM 网关（智谱地址+key+模型）→ 状态翻转已激活 → 重新打开表单回填正确（key 掌码）。
- [2026-08-12] 修复批次二（fix/l0-fk-health-onboard-ui）DONE | lint/typecheck/test/build 全绿 | 用户反馈二问题+日志发现一隐藏 bug：(1) 服务日志大量 `SQLITE_CONSTRAINT_FOREIGNKEY`（l0 appendMessages）→ 根因新空间无 agent 时代理静默降级用裸 externalId 写 L0；修复：CoreClient 新增 ensureAgent（无 agent 自动创建），pipeline 改用之；L0 路由加 FK 防御（未知 session 返 400 而非 500 刷日志）；新增 harness FK 回归用例（新空间无 agent 全链路写回落库）；(2) `/health` 返回 SPA 404 错误页 → core 新增根级 `/health` JSON 别名；UI router 新增 `*` 兜底路由与 errorElement（友好 404 页）；openapi.yaml 同步登记；(3) 设置页手动代理配置 → onboard 适配器从 proxy 迁入 core（proxy 保留转发导出），新增 `POST /api/v1/onboard/apply`（agent/proxyBaseUrl/spaceId/force/restore），设置页新增「手动接入代理」卡片（agent 下拉/代理地址/空间名/强制覆盖/接入/还原），api.ts 新增 applyOnboard；浏览器实测 /health 返 JSON、设置页卡片可用、apply 端点 200。
