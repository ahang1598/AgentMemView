import type { MemokitDatabase } from "../db/database.js";

/** A persisted bus event (row of the `events` table). */
export interface MemokitEvent {
  id: number;
  kind: string;
...
export type EventSubscriber = (event: MemokitEvent) => void;
...
export class EventBus {
  ...
  constructor(db: MemokitDatabase) {
  ...
  publish(kind: string, payload: Record<string, unknown>): number {
    ...
    for (const subscriber of this.#subscribers) {
      try {
        ...
        console.error(`[memokit] event subscriber failed for "${kind}":`, err);
      }
    }
    return event.id;
  }
  ...
  replay(sinceId: number): MemokitEvent[] {
