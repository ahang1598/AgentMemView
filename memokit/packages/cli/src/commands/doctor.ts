...
import { openDatabase } from "@memokit/core";
...
async function checkPort(label: string, port: number): Promise<CheckResult> {
  ...
  return {
    ...
    hint: `change the port in memokit.config.yaml or stop the process holding port ${port}`,
  };
}
...
function checkSqliteVec(): CheckResult {
  ...
  try {
    probeDir = mkdtempSync(path.join(tmpdir(), "memokit-doctor-vec-"));
...
function checkWritable(dataDir: string): CheckResult {
  try {
    ...
    const probe = path.join(dataDir, ".memokit-write-probe");
    ...
    return {
      status: "fail",
      detail: (err as Error).message,
      hint: "pick a writable storage.dataDir in memokit.config.yaml",
    };
  }
}
...
/** Commander action for `memokit doctor [--json]`. */
